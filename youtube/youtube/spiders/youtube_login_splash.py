...
class YoutubeVideosSpider(scrapy.Spider):
    ...
    allowed_domains = ['accounts.google.com','www.youtube.com']
    start_urls = [
    ...
    lua_script = """
        ...
        splash:set_user_agent("Mozilla/5.0 (Macintosh; Intel Mac OS X 10_12_6) AppleWebKit/604.5.6 (KHTML, like Gecko) Version/11.0.3 Safari/604.5.6")   
  	    ...
  	    splash:wait(8)
  	    splash:set_viewport_full()
  	    local loginname = splash:select('#identifierId')
  	    assert(loginname:mouse_hover{x=0, y=0})
        loginname:send_text('cctvfrench@gmail.com')
        
        assert(splash:wait(0.5))
        
        local btn = splash:select('#identifierNext > content > span')
        assert(btn:mouse_hover{x=0, y=0})
        btn:mouse_click()
        
        assert(splash:wait(7))
  
  			local loginname1 = splash:select('#password > div.aCsJod.oJeWuf > div > div.Xb9hP > input')
  	    assert(loginname1:mouse_hover{x=0, y=0})
        loginname1:send_text('super2222')
        assert(splash:wait(1))
  			
  			local btn = splash:select('#passwordNext > content > span')
        assert(btn:mouse_hover{x=0, y=0})
        btn:mouse_click()
        ...
        splash:wait(7)
  			...
  			splash:go("https://www.youtube.com/my_videos?o=U&pi=101")
  			assert(splash:wait(7))
        splash:wait(7)      
  			splash:set_viewport_full()
        return {
...
#     }
    ...
    def parse(self, response):
        ...
        fh = open('youtube_cookie_dict.txt', 'w')
        fh.write(str(cookie_dict))
        fh.close()

        fh = open('youtube_cookie_list.txt', 'w')
