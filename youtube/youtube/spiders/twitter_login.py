...
class YoutubeVideosSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        if os.path.exists('twitter_cookie_list.txt'):
            fh = open('twitter_cookie_list.txt', 'r')
            ...
            if len(cookie_txt_list):
                ...
                if len(list):
                    driver.get('https://twitter.com/login/error?username_or_email=cgtnfnewmedia%40126.com&redirect_after_login=https%3A%2F%2Fanalytics.twitter.com%2Fuser%2Fcgtnfrancais%2Ftweets%2Fexport.json%3Fstart_time%3D1522027244579%26end_time%3D1522632044579%26lang%3Dzh-cn')
        ...
        try:
            driver.find_element_by_xpath('//*[@id="page-container"]/div/div[1]/form/fieldset/div[1]/input').send_keys('cgtnfnewmedia@126.com')  # 改成你的微博账号
        except Exception as e:
            print(e)

        try:
            driver.find_element_by_xpath('//*[@id="page-container"]/div/div[1]/form/fieldset/div[2]/input').send_keys('francophone00358')  # 改成你的微博密码
        except Exception as e:
            print(e)
        ...
        try:
            driver.find_element_by_css_selector(
                '#page-container > div > div.signin-wrapper > form').click()
            driver.find_element_by_css_selector(
                '#page-container > div > div.signin-wrapper > form > div.clearfix').click()
            driver.find_element_by_css_selector('#page-container > div > div.signin-wrapper > form > div.clearfix > button').click()  # 点击登录
            driver.implicitly_wait(3)
            time.sleep(3)
        except Exception as e:
            print(e)
        ...
        try:
            ...
            js = 'document.getElementsByClassName("submit EdgeButton EdgeButton--primary EdgeButtom--medium")[0].click();'  # 点击登录
        ...
        link = driver.find_element_by_css_selector('#page-container > div > div.signin-wrapper > form > div.clearfix > button')
        link.submit()
        ActionChains(driver).move_to_element(link).perform()
        time.sleep(10)
        driver.save_screenshot('screen2.png')

        driver.find_element_by_css_selector('#page-container > div > div.signin-wrapper > form > div.clearfix > button').send_keys(Keys.SPACE)
        time.sleep(10)
