...
class WeiboListSpider(scrapy.Spider):
    ...
    start_urls = [
        'https://weibo.com/p/1002062976971705/home?is_search=0&visible=0&is_all=1&is_tag=0&profile_ftype=1&page=1#feedtop',
        ...
        for item in list:
            ...
            if len(mid_list):
                ...
                if len(count) > 3:
                    ...
                    share_property_list = item.xpath(".//ul/li[2]/a/span/span/span/em[1]/@class").extract()
                    share_list = item.xpath(".//ul/li[2]/a/span/span/span/em[2]/text()").extract()
                    if '转发' in share_list:
                        init_item['post_shares_times'] = 0
                    else:
                        if 'W_ficon ficon_forward S_ficon' in share_property_list:
                            init_item['post_shares_times'] = share_list[0]
                        elif 'W_ficon ficon_repeat S_ficon' in share_property_list:
                            if '评论' in share_list:
                                init_item['post_comments_times'] = 0
                            else:
                                init_item['post_comments_times'] = share_list[0]
                            init_item['post_shares_times'] = 0

                    # 评论
                    comments_property_list = item.xpath(".//ul/li[3]/a/span/span/span/em[1]/@class").extract()
                    comments_list = item.xpath(".//ul/li[3]/a/span/span/span/em[2]/text()").extract()
                    if '评论' in comments_list:
                        init_item['post_comments_times'] = 0
                    else:
                        if 'W_ficon ficon_repeat S_ficon' in comments_property_list:
                            init_item['post_comments_times'] = comments_list[0]
                        elif 'W_ficon ficon_praised S_txt2' in comments_property_list:
                            if '赞' in comments_list:
                                init_item['post_likes_times'] = 0
                            else:
                                init_item['post_likes_times'] = comments_list[0]
                            init_item['post_comments_times'] = 0
                    ...
                    # 评论
                    comments_property_list = item.xpath(".//ul/li[2]/a/span/span/span/em[1]/@class").extract()
                ...
                if init_item['post_shares_times'] =='评论':
                    init_item['post_shares_times'] = 0
                if init_item['post_comments_times'] =='赞':
                    init_item['post_comments_times'] = 0
    ...
    def list_parse(self, response):
        ...
        for item in list:
            ...
            if len(mid_list):
                ...
                if mid_list[0] == '4224852124047860':
                    print(mid_list[0])
                ...
                if '展开全文' in a_list:
                    #获取全部内容
                    content_url = 'https://weibo.com/p/aj/mblog/getlongtext?ajwvr=6&mid='+init_item['post_id']+'&is_settop&is_sethot&is_setfanstop&is_setyoudao&__rnd='+str(
                        int(time.time() * 1000))
                    yield scrapy.Request(
                        url=content_url,
                        callback=self.content_parse, method='GET', cookies=self.cookie,dont_filter=True)
        ...
        if 'pagebar=1' in response.url:
            ...
            yield scrapy.Request(
                url='https://weibo.com/p/aj/v6/mblog/mbloglist?ajwvr=6&domain=100206&profile_ftype=1&is_all=1&pagebar=1&pl_name=Pl_Official_MyProfileFeed__28&id=1002062976971705&script_uri=/p/1002062976971705/home&feed_type=0&page='+result+'&pre_page='+result+'&domain_op=100206&__rnd=' + str(time.time()),callback=self.list_parse, method='GET', cookies=self.cookie)
