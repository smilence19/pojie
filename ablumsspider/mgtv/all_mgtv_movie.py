...
class all_mgtv_movie():
    def __init__(self):
        print "Do spider all_mgtv_movie."
        self.program = copy.deepcopy(BASE_CONTENT["program"])
        self.program_sub = PROGRAM_SUB
        self.seedBase = ["http://list.mgtv.com/3/--------2835073-5-1-0--.html"
    ...
    def firstSpider(self, seed):
        ...
        seedJson = ""
        if mainId != '':
            seedJson = "http://pcweb.api.mgtv.com/movie/list?video_id=%s" % mainId
        else:
            return
        ...
        if json_data.get('info'):
            ...
            if json_data['info'].get('img'):
                poster = json_data['info']['img']
                prgdata = {}
                prgdata['poster'] = poster
                self.prgdata[self.program['pcUrl']] = prgdata
    ...
    def secondSpider(self):
        ...
        # add poster
        if self.prgdata.get(webUrl):
            poster = self.prgdata[webUrl]['poster']
