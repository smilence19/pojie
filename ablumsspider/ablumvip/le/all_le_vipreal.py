...
class all_le_vipreal():
    def __init__(self):
        print "Do spider all_le_vipreal."
        self.program = copy.deepcopy(BASE_CONTENT["program"])
        self.program_sub = PROGRAM_SUB
        self.seedBase = ["http://api.vip.le.com/search/interface?sales_area=cn&user_setting_country=cn&cg=16&ph=420001&pt=141001&dt=1&src=1&ispay=1&stype=1&lang=zh_cn&stt=1&ps=60&pn=1&lh=0&vt=180001%2C180011&sc=&or=6&eid=7671948092333312465"
                         , "http://api.vip.le.com/search/interface?sales_area=cn&user_setting_country=cn&cg=16&ph=420001&pt=141001&dt=1&src=1&ispay=1&stype=1&lang=zh_cn&stt=1&ps=60&pn=1&lh=0&vt=180001%2C180011&sc=&or=6&eid=7671948092333312465"
                         ]
        self.seedList = []
        self.images = {}
        ...
        self.seqNocnt = 1
        self.dataFile = spiderTool.openFile(self.dataDir + os.path.sep + "all_le_vipreal_" + self.today + ".txt")
            ...
            for each in seedList:
                ...
                if each.has_key("aid"):
                    if type(each["aid"]) is types.UnicodeType:
                        pid = each["aid"]
                        subSeed = "http://www.le.com/jilu/" + pid + ".html"
    ...
    def seedSpider(self):
        for seed in self.seedList:
            ...
            self.program["ptype"] = "记录片".decode("utf8")
    ...
    def firstSpider(self, seed):
        ...
        if re.search(r'http://www\.le\.com/jilu/(?P<pid>\d+)\.html',seed):
            pid = re.search(r'http://www\.le\.com/jilu/(?P<pid>\d+)\.html',seed).group('pid')
        elif re.search(r'http://www\.letv\.com/jilu/(?P<pid>\d+)\.html',seed):
            pid = re.search(r'http://www\.letv\.com/jilu/(?P<pid>\d+)\.html',seed).group('pid')
        if pid != '' or pid != '0':
