...
class sport_le_1():
    def __init__(self):
        print "Do spider sport_le_1."
        self.program = copy.deepcopy(BASE_CONTENT["program"])
        self.program_sub = PROGRAM_SUB
        self.seedBase = ["http://list.le.com/apin/chandata.json?c=4&d=2&md=&o=1&p=1",
                         "http://list.le.com/apin/chandata.json?c=4&d=2&md=&o=1&p=2"
        ]
        self.pidList = {}
        self.dataDir = '.' + os.path.sep + 'data'
        self.today = time.strftime('%Y%m%d', time.localtime())
        self.dataFile = spiderTool.openFile(self.dataDir + os.path.sep + "sport_le_1_" + self.today + ".txt")
    ...
    def firstSpider(self, pid, vid, detail):
        ...
        if json_data.has_key("subCategoryName"):
            ctype_P = json_data["subCategoryName"]
