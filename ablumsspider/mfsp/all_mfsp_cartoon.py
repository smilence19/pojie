...
class all_mfsp_cartoon():
    def __init__(self):
        print "Do spider all_mfsp_cartoon."
        ...
        self.dataFile = spiderTool.openFile(self.dataDir + os.path.sep + "all_mfsp_cartoon_" + self.today + ".txt")
    ...
    def doProcess(self):
        for i in range(1, 2):
            self.seedBase.append("http://www.beevideo.tv/api/video2.0/video_list.action?pageNo=" + str(i) + "&pageSize=50&channelId=5")
    ...
    def firstSpider(self, seed):
        ...
        if name == "" and name_r:
            name = re.match(r'.*!\[CDATA\[(.*)\]\]'.decode('utf-8'), str(name_r.group(1)))
            name = name.group(1)
        ...
        if poster == "" and  poster_r:
            # poster = poster_r.group(1).replace('<![CDATA[', '').replace(']]>', '').strip()
            poster = re.match(r'.*!\[CDATA\[(.*)\]\]'.decode('utf-8'), str(poster_r.group(1)))
        ...
        if shootYear == "" and  shootYear_r:
            # shootYear = shootYear_r.group(1).replace('<![CDATA[', '').replace(']]>', '').strip()
            shootYear = re.match(r'.*!\[CDATA\[(.*)\]\]'.decode('utf-8'), str(shootYear_r.group(1)))
            shootYear = shootYear.group(1)
        ...
        if area == "" and area_r:
            ...
            area = re.match(r'.*!\[CDATA\[(.*)\]\]'.decode('utf-8'), str(area_r.group(1)))
            area = area.group(1)
        ...
        if director == "" and director_r:
            # director = director_r.group(1).replace('<![CDATA[', '').replace(']]>', '').strip()
            director = re.match(r'.*!\[CDATA\[(.*)\]\]'.decode('utf-8'), str(director_r.group(1)))
        ...
        if star == "" and star_r:
            ...
            star = re.match(r'.*!\[CDATA\[(.*)\]\]'.decode('utf-8'), str(star_r.group(1)))
            star = star.group(1)
        ...
        if ctype == "" and ctype_r:
            # ctype = ctype_r.group(1).replace('<![CDATA[', '').replace(']]>', '').strip()
            ctype = re.match(r'.*!\[CDATA\[(.*)\]\]'.decode('utf-8'), str(ctype_r.group(1)))
            ctype = ctype.group(1)
        ...
        if intro == "" and intro_r:
            # intro = intro_r.group(1).replace('<![CDATA[', '').replace(']]>', '').strip()
            intro = re.match(r'.*!\[CDATA\[(.*)\]\]'.decode('utf-8'), str(intro_r.group(1)))
            intro = intro.group(1)
    ...
    def secondSpider(self, detail_sub):
        ...
        for each in data:
            ...
            setName_r = re.search(r'<annotation>(.*?)</annotation>', str(each))
            if setName_P is not None:
                setName = setName_P.get_text().replace('<![CDATA[', '').replace(']]>', '').strip()
            if setName == "" and setName_r:
                setName = re.match(r'.*!\[CDATA\[(.*)\]\]'.decode('utf-8'), str(setName_r.group(1)))
                setName = setName.group(1)
            print(setName)
            ...
            elif re.search(r'第([〇一二三四五六七八九零壹贰叁肆伍陆柒捌玖貮两十拾百佰千仟万萬亿億兆]+)'.decode('utf8'), setName):
                setNumberH = re.search(r'第([〇一二三四五六七八九零壹贰叁肆伍陆柒捌玖貮两十拾百佰千仟万萬亿億兆]+)'.decode('utf8'), setName).group(1)
