...
class all_le_movie():
    def __init__(self):
        ...
        self.seedBase = ["http://list.le.com/apin/chandata.json?c=1&d=1&md=&o=4&p=1&s=1",
                         "http://list.le.com/apin/chandata.json?c=1&d=1&md=&o=4&p=1&s=1",
                         "http://list.le.com/apin/chandata.json?c=1&d=1&md=&o=4&p=2&s=1",
                         "http://list.le.com/apin/chandata.json?c=1&d=1&md=&o=4&p=3&s=1",
                         "http://list.le.com/apin/chandata.json?c=1&d=1&md=&o=4&p=4&s=1",
                         "http://list.le.com/apin/chandata.json?c=1&d=1&md=&o=4&p=5&s=1",
                         "http://list.le.com/apin/chandata.json?c=1&d=1&md=&o=4&p=6&s=1",
        ]
        self.seedList = []
        self.images = {}
        ...
        self.jsonData = ""
    #     self.dataFile = spiderTool.openFile(self.dataDir + os.path.sep + "all_le_movie_" + self.today + ".txt")
    #
    # def __del__(self):
            ...
            for each in seedList:
                ...
                if each.has_key("aid"):
                    if type(each["aid"]) is types.UnicodeType:
                        pid = each["aid"]
                        subSeed = "http://www.le.com/movie/" + pid + ".html"
    ...
    def firstSpider(self, seed):
        ...
        pid = ""
        if re.search(r'http://www\.le\.com/movie/(?P<pid>\d+)\.html',seed):
            pid = re.search(r'http://www\.le\.com/movie/(?P<pid>\d+)\.html',seed).group('pid')
        elif re.search(r'http://www\.letv\.com/movie/(?P<pid>\d+)\.html',seed):
            pid = re.search(r'http://www\.letv\.com/movie/(?P<pid>\d+)\.html',seed).group('pid')
        if pid != '' or pid != '0':
    ...
    def secondSpider(self, seed):
        ...
        for each in data:
            ...
            if setNumber == "" and len(data) == 1:
                setNumber = '1'
            if each.has_key('nameCn'):
                setName = each['nameCn']
            if setName == '' and each.has_key('subTitle'):
