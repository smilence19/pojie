...
class all_le_tv():
    def __init__(self):
        print "Do spider all_le_tv."
        self.program = copy.deepcopy(BASE_CONTENT["program"])
        self.program_sub = PROGRAM_SUB
        self.seedBase = ["http://list.le.com/apin/chandata.json?c=2&d=1&md=&o=51&p=1&s=1",
                         "http://list.le.com/apin/chandata.json?c=2&d=1&md=&o=20&p=1&s=1",
                         "http://list.le.com/apin/chandata.json?c=2&d=1&md=&o=20&p=2&s=1",
                         "http://list.le.com/apin/chandata.json?c=2&d=1&md=&o=20&p=3&s=1",
                         "http://list.le.com/apin/chandata.json?c=2&d=1&md=&o=20&p=4&s=1",
                         "http://list.le.com/apin/chandata.json?c=2&d=1&md=&o=20&p=5&s=1",
                         "http://list.le.com/apin/chandata.json?c=2&d=1&md=&o=20&p=6&s=1",
        ]
        self.seedList = []
        self.images = {}
        ...
        self.jsonData = ""
    #     self.dataFile = spiderTool.openFile(self.dataDir + os.path.sep + "all_le_tv_" + self.today + ".txt")
    #
    # def __del__(self):
            ...
            for each in seedList:
                ...
                if each.has_key("aid"):
                    if type(each["aid"]) is types.UnicodeType:
                        pid = each["aid"]
                        subSeed = "http://www.le.com/tv/" + pid + ".html"
    ...
    def firstSpider(self, seed):
        ...
        pid = ""
        if re.search(r'http://www\.le\.com/tv/(?P<pid>\d+)\.html',seed):
            pid = re.search(r'http://www\.le\.com/tv/(?P<pid>\d+)\.html',seed).group('pid')
        elif re.search(r'http://www\.letv\.com/tv/(?P<pid>\d+)\.html',seed):
            pid = re.search(r'http://www\.letv\.com/tv/(?P<pid>\d+)\.html',seed).group('pid')
        if pid != '' or pid != '0':
